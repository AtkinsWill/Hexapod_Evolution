...
public class Simulation_Control : MonoBehaviour
{
    ...
    void Start()
    {
        using (StreamWriter streamWriter = File.AppendText("CSVdataOutput/fitnesses.csv"))
        {
            streamWriter.WriteLine("Iteration,Max Fitness,Mean Fitness");
        }
    }
...
void InitaliseRandomWeights()
{
    ...
    void evaluate()
    {
        ...
        for (int i = 0; i < numHexapods; i++)
        {
            ...
            hexapodFitnesses[i] = hexapodFitnesses[i] * hexapod_control.getXVelocity();
        ...
        for (int i = 0; i < numHexapods; i++)
        {
            // Move the initialization inside the loop for each hexapod
    ...
    void saveFitness(float max, float average)
    {
        using (StreamWriter streamWriter = File.AppendText("CSVdataOutput/fitnesses.csv"))
        {
            streamWriter.WriteLine(gencounter.ToString() + "," + max.ToString() + "," + average.ToString());
        }
    }


    public void playTheSim()
    {
        Debug.Log("Run sim");
        runSim = true;
    }
