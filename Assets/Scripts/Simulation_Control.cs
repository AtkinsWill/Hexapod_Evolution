            ...
            for (int k = 0; k < num_neuron_per_set; k++)
            {
                for (int l = 0; l < num_neuron_per_set; l++)
                {
                    ...
                    tempInputWeights[j, k, l] = random.NextDouble() * 2 - 1;
                        //PrintWeights(tempIntraNeuronWeights);
                        //PrintWeightsCSV(tempIntraNeuronWeights, "CSVdataOutput/HexapodID" + i + "_step1_InitRandom.csv");
                        
                        //print(tempInputWeights[j, k, l]);
                    }
                    tempExoNeuronWeights[j, k] = random.NextDouble() * 2 - 1;
                    //print(tempExoNeuronWeights[j, k]);
                }
        }
    ...
    void evaluate()
    {
        ...
        for (int i = 0; i < numHexapods; i += 2)
        {
            if (i + 1 < numHexapods)
            {
                ...
                //PrintWeightsCSV4D(newAllIntraNW, i, "CSVdataOutput/HexapodID" + i + "_step4_PostCrossover.csv");
                //int eye = i++;
                //PrintWeightsCSV4D(newAllIntraNW, eye, "CSVdataOutput/HexapodID" + eye + "_step4_PostCrossover.csv");
